...
public class FeatureFlagCommandController {
  ...
  public ResponseEntity<Void> renameFlag(
      @PathVariable UUID flagId, @RequestBody @Valid RenameFlagBody body) {
  ...
  public ResponseEntity<Void> addEnvironment(
      @PathVariable UUID flagId, @RequestBody @Valid AddEnvironmentBody body) {
    handler.handle(new AddEnvironmentCommand(flagId, body.name()));
    return ResponseEntity.created(URI.create("/flags/" + flagId + "/environments/" + body.name()))
        .build();
  }

  @DeleteMapping("/{flagId}/environments/{environmentName}")
  public ResponseEntity<Void> removeEnvironment(
      @PathVariable UUID flagId, @PathVariable String environmentName) {
    handler.handle(new RemoveEnvironmentCommand(flagId, environmentName));
    return ResponseEntity.noContent().build();
  }

  @PostMapping("/{flagId}/variants")
  public ResponseEntity<Map<String, UUID>> addVariant(
      @PathVariable UUID flagId, @RequestBody @Valid AddVariantBody body) {
  ...
  public ResponseEntity<Void> removeVariant(
      @PathVariable UUID flagId, @PathVariable UUID variantId) {
