...
public class FeatureFlagCommandHandler {
  ...
  public UUID handle(CreateFeatureFlagCommand cmd) {
    ...
    eventStore.append(
        flagId, List.of(new FeatureFlagCreatedEvent(flagId, cmd.name(), Instant.now())), 0);
    return flagId;
  }
  ...
  public void handle(RenameFeatureFlagCommand cmd) {
    ...
    eventStore.append(
        cmd.flagId(),
        List.of(
            new FeatureFlagRenamedEvent(
                cmd.flagId(), loaded.flag().name(), cmd.newName(), Instant.now())),
        loaded.version());
  }
  ...
  public void handle(AddEnvironmentCommand cmd) {
    ...
    eventStore.append(
        cmd.flagId(),
        List.of(new EnvironmentAddedEvent(cmd.flagId(), cmd.environmentName(), Instant.now())),
        loaded.version());
  }
  ...
  public void handle(RemoveEnvironmentCommand cmd) {
    ...
    eventStore.append(
        cmd.flagId(),
        List.of(new EnvironmentRemovedEvent(cmd.flagId(), cmd.environmentName(), Instant.now())),
        loaded.version());
  }
  ...
  public void handle(RemoveVariantCommand cmd) {
    ...
    eventStore.append(
        cmd.flagId(),
        List.of(new VariantRemovedEvent(cmd.flagId(), cmd.variantId(), Instant.now())),
        loaded.version());
  }
  ...
  public void handle(ModifyVariantCommand cmd) {
    ...
    eventStore.append(
        cmd.flagId(),
        List.of(
            new VariantModifiedEvent(
                cmd.flagId(), cmd.variantId(), cmd.name(), cmd.value(), Instant.now())),
        loaded.version());
  }
